    ...
    public class ReportService : IReportService
    {
        ...
        public async Task<List<AttendanceReportViewModel>> GetAttendanceReport(ReportViewModel reportViewModel)
        {
            ...
            for (int i = 0; i < days_count; i++) // IO Loop
            {
                ...
                {
                    #region Preparing
                    string io_day = ToDateTime(reportViewModel.IOs[p].TTime).ToString("dddd"); // ÌÊ„ «·Õ—ﬂ…



                    string x = ToDateTime(reportViewModel.IOs[p].TTime).ToString("yyyy-MM-dd"); //  «—ÌŒ «·Õ—ﬂ…
                    DateTime m = ToDateTime(reportViewModel.IOs[p].TTime); //  «—ÌŒ ÊÊﬁ  «·Õ—ﬂ…
                    int m_action = m.Hour;
                    ...
                    if (m_action >= 24)
                    {
                        string io_prevday = ToDateTime(reportViewModel.IOs[p].TTime).AddDays(-1).ToString("dddd"); // «·ÌÊ„ «·”«»ﬁ
                    ...
                    if (ti == true && Fixed(day, reportViewModel.Worksys) == false) // ÌÕœœ ‰Ê⁄ «·Õ—ﬂ… »Õ”» «·Êﬁ  Ê‰Ÿ«„ À«» 
                    {
                        ...
                        if (reportViewModel.IOs[p].Event.ToString() == "0" && m_action >= First_as && m_action <= First_ae && x == y) // Õ÷Ê— œÊ«„ 1
                        {
                            ...
                            {
                                dt_att.Rows[i]["Õ÷Ê— œÊ«„ 1"] = ToDateTime(reportViewModel.IOs[p].TTime).ToString(sh);
                        ...
                        else if (reportViewModel.IOs[p].Event.ToString() == "1" && m_action >= First_ls && m_action <= First_le && m >= ds && m <= de && t(dt_att.Rows[i], 1, m) > TimeSpan.Parse("0.00:00:00")) // ≈‰’—«› œÊ«„ 1
                        {
                            ...
                            dt_att.Rows[i]["≈‰’—«› œÊ«„ 1"] = ToDateTime(reportViewModel.IOs[p].TTime).ToString(s);
                        ...
                        else if (reportViewModel.IOs[p].Event.ToString() == "0" && m_action >= Second_as && m_action <= Second_ae && x == y) // Õ÷Ê— œÊ«„ 2
                        {
                            ...
                            {
                                dt_att.Rows[i]["Õ÷Ê— œÊ«„ 2"] = ToDateTime(reportViewModel.IOs[p].TTime).ToString(s);
                        ...
                        else if (reportViewModel.IOs[p].Event.ToString() == "1" && m_action >= Second_ls && m_action <= Second_le && m >= ds && m <= de && t(dt_att.Rows[i], 2, m) > TimeSpan.Parse("0.00:00:00")) // ≈‰’—«› œÊ«„ 2
                        {
                            ...
                            string s = (ToDateTime(reportViewModel.IOs[p].TTime).ToString(sd) == y) ? sh : l;
                            dt_att.Rows[i]["≈‰’—«› œÊ«„ 2"] = ToDateTime(reportViewModel.IOs[p].TTime).ToString(s);
                    ...
                    else  // ·« ÌÕœœ ‰Ê⁄ «·Õ—ﬂ… »Õ”» «·Êﬁ  √Ê ‰Ÿ«„ €Ì— À«» 
                    {
                        ...
                        {
                            string s = (ToDateTime(reportViewModel.IOs[p].TTime).ToString(sd) == y) ? sh : l;
                            dt_att.Rows[i]["Õ÷Ê— œÊ«„ 2"] = (dt_att.Rows[i]["Õ÷Ê— œÊ«„ 2"].ToString() == "") ? ToDateTime(reportViewModel.IOs[p].TTime).ToString(s) : dt_att.Rows[i]["Õ÷Ê— œÊ«„ 2"].ToString();
